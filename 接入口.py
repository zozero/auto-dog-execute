...
from 数据类型屋.接收类型 import 执行数据类, 图片匹配数据类, 图片二值化匹配数据类, 步骤数据类, 测试步骤数据类, \
    任务数据类, 测试任务数据类, 匹配再匹配数据类, 无图匹配数据类, 多图匹配数据类, 你只看一次数据类, 简单光学字符识别数据类
...
@快捷应用程序接口.get("/简单光学字符识别/测试")
def 简单光学字符识别测试(模拟器的ip和端口: str, 范围: str, 语种: int):
    范围 = tuple(map(int, 范围.strip().split(' ')))
    我的模拟器 = 安卓指令类(模拟器的ip和端口)

    我的图片路径, 我的图片 = 我的模拟器.裁剪图片(范围)

    图片 = Image.fromarray(cv2.cvtColor(我的图片, cv2.COLOR_BGR2RGB))
    图片 = np.asarray(图片)
    # 载入图片
    # 图片 = Image.open(图片路径)
    语种 = 语种列表[语种]
